...
class sale_report_wizard(models.TransientModel):
    ...
    def print_exl_report(self):
        if xlwt:
            ...
            worksheet.write_merge(0, 1, 0, 19, "Sales Report", style=style_title)
            	...
            	(	SELECT 
                      sum(sol.product_uom_qty*sol.price_unit) as mtd_dispatch_value,
                         so.user_id AS user_id_val,   
                           sum(so.amount_total) AS mtd_dispatch_collections,    
                          sum(CASE WHEN pp.freebie=false or pp.freebie is null THEN (sol.product_uom_qty) ELSE 0.0 END) as mtd_dispatch_units
...
class Sale_Report_excel(models.TransientModel):
    ...
    excel_file = fields.Binary('Excel Report For Sale report ')
