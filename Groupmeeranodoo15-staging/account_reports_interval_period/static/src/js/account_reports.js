odoo.define('account_reports_interval_period.account_report_interval', function (require) {
'use strict';
...
//var _super_orderline = AccountReports.accountReportsWidget.prototype;
//models.accountReportsWidget = models.accountReportsWidget.extend({
//
//
//});
...
//                var interval = self.$searchview_buttons.find('.o_interval_input[name="interval"]');
//                if (interval.length > 0){
//                    console.log(interval.val());
//                    self.report_options.date.interval = field_utils.parse.integer(interval.val());
//                }
...
//    },
    render_searchview_buttons: function() {
        this._super();
        var self = this;
        console.log(self.report_options.date.interval);
        // bind searchview buttons/filter to the correct actions
        var $interval = this.$searchview_buttons.find('.js_account_reports_interval');
        var name = $interval.find('input').attr('name');
        var defaultValue = $interval.data('default-value');
        self.report_options.date.interval = defaultValue;

        //interval2
        var $interval2 = this.$searchview_buttons.find('.js_account_reports_interval2');
        var name2 = $interval2.find('input').attr('name');
        var defaultValue2 = $interval2.data('default-value');
        self.report_options.date.interval2 = defaultValue2;

        //interval3
        var $interval3 = this.$searchview_buttons.find('.js_account_reports_interval3');
        var name3 = $interval3.find('input').attr('name');
        var defaultValue3 = $interval3.data('default-value');
        self.report_options.date.interval3 = defaultValue3;

        //interval4
        var $interval4 = this.$searchview_buttons.find('.js_account_reports_interval4');
        var name4 = $interval4.find('input').attr('name');
        var defaultValue4 = $interval4.data('default-value');
        self.report_options.date.interval4 = defaultValue4;

//        var input_interval = $interval.find('input').attr('name', name)
//        var input_interval = document.getElementById("o_interval_input");
//        input_interval.value = defaultValue;
        ...
        this.$searchview_buttons.find('.js_account_report_date_filter').click(function (event) {
            ...
            if ($(this).data('filter') === 'custom') {
                ...
                var date_to = self.$searchview_buttons.find('.o_datepicker_input[name="date_to"]');
                var interval = self.$searchview_buttons.find('.o_interval_input[name="interval"]');
                var interval2 = self.$searchview_buttons.find('.o_interval_input2[name="interval2"]');
                var interval3 = self.$searchview_buttons.find('.o_interval_input3[name="interval3"]');
                var interval4 = self.$searchview_buttons.find('.o_interval_input4[name="interval4"]');
                if (interval.length > 0){
                    console.log(interval.val());
                    self.report_options.date.interval = field_utils.parse.integer(interval.val());
                }
                if (interval2.length > 0){
                    ...
                    self.report_options.date.interval2 = field_utils.parse.integer(interval2.val());
                }
                if (interval3.length > 0){
                    console.log(interval3.val());
                    self.report_options.date.interval3 = field_utils.parse.integer(interval3.val());
                }
                if (interval4.length > 0){
                    console.log(interval4.val());
                    self.report_options.date.interval4 = field_utils.parse.integer(interval4.val());
                }
                if (date_from.length > 0){
                    error = date_from.val() === "" || date_to.val() === "";
                    self.report_options.date.date_from = field_utils.parse.date(date_from.val());
