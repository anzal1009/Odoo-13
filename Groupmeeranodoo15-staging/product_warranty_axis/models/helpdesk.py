...
from odoo.addons.product_warranty_axis.models.helpdesk_ticket import TICKET_PRIORITY
...
class HelpdeskTeam(models.Model):
    ...
    def _default_stage_ids(self):
        ...
        return [(0, 0, {'name': _('New'), 'sequence': 0, 'template_id': self.env.ref('product_warranty_axis.new_ticket_request_email_template', raise_if_not_found=False) or None})]

    def _default_domain_member_ids(self):
        return [('groups_id', 'in', self.env.ref('product_warranty_axis.group_helpdesk_user').id)]
    ...
    def _check_sla_group(self):
        for team in self:
            if team.use_sla and not self.user_has_groups('product_warranty_axis.group_use_sla'):
                self.env.ref('product_warranty_axis.group_helpdesk_user').write({'implied_ids': [(4, self.env.ref('product_warranty_axis.group_use_sla').id)]})
            if team.use_sla:
                ...
                if not self.search_count([('use_sla', '=', True)]):
                    self.env.ref('product_warranty_axis.group_helpdesk_user').write({'implied_ids': [(3, self.env.ref('product_warranty_axis.group_use_sla').id)]})
                    self.env.ref('product_warranty_axis.group_use_sla').write({'users': [(5, 0, 0)]})
            ...
            if team.use_rating:
                for stage in team.stage_ids:
                    if stage.is_close and not stage.fold:
                        stage.template_id = self.env.ref('product_warranty_axis.rating_ticket_request_email_template', raise_if_not_found= False)
    ...
    def retrieve_dashboard(self):
        ...
        user_uses_sla = self.user_has_groups('product_warranty_axis.group_use_sla') and\
        ...
        closed_tickets = HelpdeskTicket.search_read(expression.AND([domain, [('stage_id.is_close', '=', True)]]),
                                                    ['sla_deadline', 'close_hours', 'sla_reached_late', 'priority'])
        ...
        result = {
            ...
            'pending_ticket': {'count': 0, 'hours': 0, 'failed': 0},
        ...
        result['pending_ticket']['hours'] = round(result['pending_ticket']['hours'] / (result['pending_ticket']['count'] or 1), 2)
    ...
    def action_view_ticket(self):
        action = self.env["ir.actions.actions"]._for_xml_id("product_warranty_axis.helpdesk_ticket_action_team")
