...
class HelpdeskTicket(models.Model):
    ...
    user_id = fields.Many2one(
        ...
        domain=lambda self: [('groups_id', 'in', self.env.ref('product_warranty_axis.group_helpdesk_user').id)])
    ...
    def _compute_domain_user_ids(self):
        for task in self:
            if task.team_id and task.team_id.visibility_member_ids:
                helpdesk_manager = self.env['res.users'].search([('groups_id', 'in', self.env.ref('product_warranty_axis.group_helpdesk_manager').id)])
                task.domain_user_ids = [(6, 0, (helpdesk_manager + task.team_id.visibility_member_ids).ids)]
            else:
                helpdesk_users = self.env['res.users'].search([('groups_id', 'in', self.env.ref('product_warranty_axis.group_helpdesk_user').id)]).ids
    ...
    def create_action(self, action_ref, title, search_view_ref):
        ...
        # calculate_all= self.sudo().search_count([('active', '=', False),('probability', '=', 0)])
    ...
    def create(self, list_value):
        ...
        template = self.env.ref('product_warranty_axis.new_ticket_request_email_template')
        mail= template.send_mail(tickets.id, True)
    ...
    def _creation_subtype(self):
        return self.env.ref('product_warranty_axis.mt_ticket_new')

    def _track_subtype(self, init_values):
        self.ensure_one()
        if 'stage_id' in init_values:
            return self.env.ref('product_warranty_axis.mt_ticket_stage')
    ...
    def _notify_get_groups(self, msg_vals):
        ...
        helpdesk_user_group_id = self.env.ref('product_warranty_axis.group_helpdesk_user').id
        new_groups = [(
            'group_helpdesk_user',
    ...
    def rating_apply(self, rate, token=None, feedback=None, subtype_xmlid=None):
        return super(HelpdeskTicket, self).rating_apply(rate, token=token, feedback=feedback, subtype_xmlid="product_warranty_axis.mt_ticket_rated")
